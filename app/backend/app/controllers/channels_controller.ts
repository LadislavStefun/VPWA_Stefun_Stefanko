...
export default class ChannelsController {
  public async store({ request, auth, response }: HttpContext) {
    ...
    const existing = await Channel.query().where('name', name).whereNull('deleted_at').first()
  ...
  public async joinByName({ request, auth, response }: HttpContext) {
    ...
    let channel = await Channel.query().where('name', name).whereNull('deleted_at').first()
    ...
    console.log(`joinByName for user ${user.id} took ${duration}ms`)
